...
static command_response_t _hw_pupd_cb(char* args, cmd_output_t cmd_output)
{
    char* p;
    ...
    if (io != W1_PULSE_1_IO &&
        io != W1_PULSE_2_IO)
        cmd_output("Selected IO is not W1 or pulse IO.");
    ...
    cmd_output("IO %u: %"PRIu8, io, (uint8_t)(enabled?1:0));
