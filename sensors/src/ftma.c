...
static command_response_t _ftma_name_cb(char* args, cmd_output_t cmd_output)
{
    ...
    *new_name = '\0';
    ...
    {
        cmd_output("Failed to get FTMA with name '%s'.", orig_name);
        return COMMAND_RESP_ERR;
    }
    if (index > ADC_FTMA_COUNT)
    {
        cmd_output("Index is out of range.");
        return COMMAND_RESP_ERR;
    }
    unsigned new_len = strlen(new_name);
    if (new_len > MEASURE_NAME_LEN)
    {
        cmd_output("Max name length is %d, you tried length %u", MEASURE_NAME_LEN, new_len);
        return COMMAND_RESP_ERR;
    }
    if (new_len == 0)
    {
    ...
    for (char* c = new_name; *c; c++)
    {
        if (!isascii(*c))
        {
            cmd_output("New name '%s' contains none ascii characters.", new_name);
            return COMMAND_RESP_ERR;
        }
    }
    if (!measurements_rename(orig_name, new_name))
    {
        cmd_output("Failed to rename the measurement.");
        return COMMAND_RESP_ERR;
    }
    strncpy(_ftma_config[index].name, new_name, MEASURE_NAME_LEN);
    cmd_output("Measurement '%s' is now called '%s'.", orig_name, new_name);
    return COMMAND_RESP_OK;
}


static command_response_t _ftma_coeff_cb(char* args, cmd_output_t cmd_output)
{
    /* <name>  <A>  <B>    <C>      <D>
    ...
    {
        cmd_output("Failed to get FTMA with name '%s'.", args);
        return COMMAND_RESP_ERR;
    }
    if (index > ADC_FTMA_COUNT)
    {
    ...
    if (!just_print)
    {
        ...
        if (np != p)
        {
            ...
            cmd_output("Set new coefficients for '%s'", args);
        }
    }

    cmd_output("Coeffients for '%s':", args);
    for (unsigned i = 0; i < FTMA_NUM_COEFFS; i++)
    {
        cmd_output("%c: %.06f", 'A'+i, ftma->coeffs[i]);
    }
    return COMMAND_RESP_OK;
}
