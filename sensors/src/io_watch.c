...
bool io_watch_enable(unsigned io, bool enabled, io_pupd_t pupd)
{
    if (enabled)
    {
        if (io >= IOS_WATCH_COUNT)
            return false;
        /* Setup pins and interrupt to both? */

        port_n_pins_t* pins = (port_n_pins_t*)&ios_pins[io];
        platform_gpio_init(pins);
        platform_gpio_setup(pins, true, pupd);
        //platform_gpio_setup_exti(pins,
