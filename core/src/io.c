...
void     io_log(unsigned io, cmd_output_t cmd_output)
{
    if (io >= ARRAY_SIZE(ios_pins))
        return;
    ...
    if (!(io_state & IO_ACTIVE_SPECIAL_MASK))
    {
        ...
        if (io_state & IO_AS_INPUT)
            cmd_output("IO %02u : %s%s%sIN %s = %s",
                       io, pretype, type, posttype,
                       io_get_pull_str(io_state),
                       (platform_gpio_get(gpio_pin))?"ON":"OFF");
        else
            cmd_output("IO %02u : %s%s%sOUT %s = %s",
                       io, pretype, type, posttype,
                       io_get_pull_str(io_state),
                       (platform_gpio_get(gpio_pin))?"ON":"OFF");
    }
    else
    {
        ...
        if ((io_state & IO_PULL_MASK) == GPIO_PUPD_PULLUP)
            ...
            pupd_char = ' ';
        cmd_output("IO %02u : %s%s%sUSED %s %c", io, pretype, type, posttype, active_type, pupd_char);
    }
}


void     ios_log(cmd_output_t cmd_output)
{
    for(unsigned n = 0; n < ARRAY_SIZE(ios_pins); n++)
        io_log(n, cmd_output);
}
...
static command_response_t _io_log_cb(char* args, cmd_output_t cmd_output)
{
    ios_log(cmd_output);
...
static command_response_t _io_cb(char *args, cmd_output_t cmd_output)
{
    /* io 0 : OUT = NONE
     * io 1 : IN = DOWN
     * io 2 = ON
    ...
    if (*pos == ':')
    {
        ...
        {
            cmd_error("Malformed gpio type command");
        ...
        if (*pos && *pos != '=')
        {
            ...
            {
                cmd_error("Malformed gpio pull command");
                return COMMAND_RESP_ERR;
            }
            pos = skip_space(pos);
        }
    ...
    if (*pos == '=')
    {
        ...
        if (strncmp(pos, "ON", 2) == 0 || *pos == '1')
        {
            ...
            if (!io_is_input(io))
            {
                io_on(io, true);
                if (!do_read)
                    cmd_output("IO %02u = ON", io);
            }
            else cmd_error("IO %02u is input but output command.", io);
        }
        else if (strncmp(pos, "OFF", 3) == 0 || *pos == '0')
        {
            ...
            if (!io_is_input(io))
            {
                io_on(io, false);
                if (!do_read)
                    cmd_output("IO %02u = OFF", io);
            }
            else cmd_error("IO %02u is input but output command.", io);
        }
        else
        {
            cmd_error("Malformed gpio on/off command");
            return COMMAND_RESP_ERR;
        }
    }
    else do_read = true;
    ...
    if (do_read)
        io_log(io, cmd_output);
    return COMMAND_RESP_OK;
}


static command_response_t _io_cmd_enable_pulsecount_cb(char * args, cmd_output_t cmd_output)
{
    /* <io> <R/F/B> <U/D/N>
     */
    char * pos = NULL;
    ...
    if (io_enable_pulsecount(io, pupd, edge))
        cmd_output("IO %02u pulsecount enabled", io);
    else
        cmd_output("IO %02u has no pulsecount", io);
    return COMMAND_RESP_OK;
bad_exit:
    cmd_output("<io> <R/F/B> <U/D/N>");
    return COMMAND_RESP_ERR;
}


static command_response_t _io_cmd_enable_onewire_cb(char * args, cmd_output_t cmd_output)
{
    char * pos = NULL;
    unsigned io = strtoul(args, &pos, 10);
    ...
    if (io_enable_w1(io))
    {
        cmd_output("IO %02u onewire enabled", io);
        return COMMAND_RESP_OK;
    }

    cmd_output("IO %02u has no onewire", io);
    return COMMAND_RESP_ERR;
}


static command_response_t _io_cmd_enable_watch_cb(char* args, cmd_output_t cmd_output)
{
    /* <io> <U/D/N>
     */
    char * pos = NULL;
    ...
    if (io_enable_watch(io, pupd))
        cmd_output("IO %02u watch enabled", io);
    else
        cmd_output("IO %02u has no watch", io);
    return COMMAND_RESP_OK;
bad_exit:
    cmd_output("<io> <U/D/N>");
    return COMMAND_RESP_ERR;
