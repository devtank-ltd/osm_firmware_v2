 ...
 *        false if same      */
bool osm_model_persist_config_cmp(persist_model_config_v1_t* d0, persist_model_config_v1_t* d1)
{
    return !(
        d0 && d1 &&
...
bool osm_model_measurements_get_inf(measurements_def_t * def, measurements_data_t* data, measurements_inf_t* inf)
{
    ...
    switch(def->type)
    {
        ...
        case FTMA:          osm_ftma_inf_init(inf);        break;
        default:
...
void osm_model_measurements_repopulate(void)
{
    ...
    osm_measurements_repop_indiv(MEASUREMENTS_LIGHT_NAME,           1,  1,  LIGHT           );
    ...
    osm_measurements_repop_indiv(MEASUREMENTS_FTMA_1_NAME,          1,  1,  FTMA            );}
...
unsigned osm_model_measurements_add_defaults(measurements_def_t * measurements_arr)
{
    ...
    osm_measurements_setup_default(&measurements_arr[pos++], MEASUREMENTS_FTMA_1_NAME,          1,  1,  FTMA           );
    return pos;
}
