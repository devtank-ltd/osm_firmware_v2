...
void osm_model_persist_config_model_init(osm_persist_model_config_t* model_config)
{
    ...
    osm_cc_setup_default_mem(model_config->cc_configs, sizeof(osm_cc_config_t) * ADC_CC_COUNT);
    osm_ftma_setup_default_mem(model_config->ftma_configs, sizeof(osm_ftma_config_t) * ADC_FTMA_COUNT);
...
bool osm_model_persist_config_cmp(osm_persist_model_config_t* d0, osm_persist_model_config_t* d1)
{
    return !(
        ...
        !osm_at_wifi_persist_config_cmp(&d0->comms_config, &d1->comms_config) &&
...
void osm_model_measurements_repopulate(void)
{
    ...
    osm_measurements_repop_indiv(OSM_MEASUREMENTS_PM10_NAME,            1,  5,  OSM_PM10            );
    osm_measurements_repop_indiv(OSM_MEASUREMENTS_PM25_NAME,            1,  5,  OSM_PM25            );
    osm_measurements_repop_indiv(OSM_MEASUREMENTS_PM1_0_NAME,           1,  5,  OSM_SENxx           );
    osm_measurements_repop_indiv(OSM_MEASUREMENTS_PM4_NAME,             1,  5,  OSM_SENxx           );
    osm_measurements_repop_indiv(OSM_MEASUREMENTS_REL_HUM_NAME,         1,  5,  OSM_SENxx           );
    osm_measurements_repop_indiv(OSM_MEASUREMENTS_SEN5x_TEMP_NAME,      1,  5,  OSM_SENxx           );
    osm_measurements_repop_indiv(OSM_MEASUREMENTS_VOC_NAME,             1,  5,  OSM_SENxx           );
    osm_measurements_repop_indiv(OSM_MEASUREMENTS_NOX_NAME,             1,  5,  OSM_SENxx           );
    ...
    osm_measurements_repop_indiv(OSM_MEASUREMENTS_FTMA_1_NAME,          0,  25, OSM_FTMA            );
    osm_measurements_repop_indiv(OSM_MEASUREMENTS_FTMA_2_NAME,          0,  25, OSM_FTMA            );
    osm_measurements_repop_indiv(OSM_MEASUREMENTS_FTMA_3_NAME,          0,  25, OSM_FTMA            );
    osm_measurements_repop_indiv(OSM_MEASUREMENTS_FTMA_4_NAME,          0,  25, OSM_FTMA            );
...
void osm_model_cmds_add_all(struct osm_cmd_link_t* tail)
{
    ...
    tail = osm_linux_add_commands(tail);
}
...
unsigned osm_model_measurements_add_defaults(osm_measurements_def_t * measurements_arr)
{
    ...
    osm_measurements_setup_default(&measurements_arr[pos++], OSM_MEASUREMENTS_PM1_0_NAME,           1,  5,  OSM_SENxx           );
    osm_measurements_setup_default(&measurements_arr[pos++], OSM_MEASUREMENTS_PM4_NAME,             1,  5,  OSM_SENxx           );
    osm_measurements_setup_default(&measurements_arr[pos++], OSM_MEASUREMENTS_REL_HUM_NAME,         1,  5,  OSM_SENxx           );
    osm_measurements_setup_default(&measurements_arr[pos++], OSM_MEASUREMENTS_SEN5x_TEMP_NAME,      1,  5,  OSM_SENxx           );
    osm_measurements_setup_default(&measurements_arr[pos++], OSM_MEASUREMENTS_VOC_NAME,             1,  5,  OSM_SENxx           );
    osm_measurements_setup_default(&measurements_arr[pos++], OSM_MEASUREMENTS_NOX_NAME,             1,  5,  OSM_SENxx           );
    osm_measurements_setup_default(&measurements_arr[pos++], OSM_MEASUREMENTS_CO2_NAME,             0,  5,  OSM_SENxx           );
    osm_measurements_setup_default(&measurements_arr[pos++], OSM_MEASUREMENTS_HCHO_NAME,            0,  5,  OSM_SENxx           );
    ...
    osm_measurements_setup_default(&measurements_arr[pos++], OSM_MEASUREMENTS_FTMA_1_NAME,          0,  25, OSM_FTMA            );
    osm_measurements_setup_default(&measurements_arr[pos++], OSM_MEASUREMENTS_FTMA_2_NAME,          0,  25, OSM_FTMA            );
    osm_measurements_setup_default(&measurements_arr[pos++], OSM_MEASUREMENTS_FTMA_3_NAME,          0,  25, OSM_FTMA            );
    osm_measurements_setup_default(&measurements_arr[pos++], OSM_MEASUREMENTS_FTMA_4_NAME,          0,  25, OSM_FTMA            );
...
{
    *pid = osm_linux_spawn(FAKE_AT_WIFI_SERVER);
}


void osm_model_linux_spawn_fakes(void)
{
    osm_peripherals_add_cmd(&_model_pids[0]);
    osm_peripherals_add_modbus(EXT_UART , &_model_pids[1]);
    osm_peripherals_add_hpm(HPM_UART    , &_model_pids[2]);
    osm_peripherals_add_w1(10000000      , &_model_pids[3]);
    osm_peripherals_add_i2c(20000000     , &_model_pids[4]);
    peripherals_add_at_wifi(&_model_pids[5]);
    osm_peripherals_add_example_rs232(RS232_UART , &_model_pids[6]);
}
