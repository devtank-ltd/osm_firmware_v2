...
static void _comms_direct_exit(void)
{
    ...
    osm_uarts_setup();
    osm_uart_enable(CMD_UART, true);
    osm_uart_enable(COMMS_UART, true);
    osm_uart_rings_in_wipe(CMD_UART);
    osm_uart_rings_in_wipe(COMMS_UART);
    osm_log_out("Exiting COMMS_DIRECT mode");
}


static bool _comms_direct_loop_iteration(void)
{
    const bool cmd_rx = osm_platform_gpio_get(&_comms_direct_ctx.cmd_rx_ports_n_pins);
    const bool comms_rx = osm_platform_gpio_get(&_comms_direct_ctx.comms_rx_ports_n_pins);

    uint32_t now = osm_get_since_boot_ms();
    if (cmd_rx   != _comms_direct_ctx.prev_cmd_rx)
    {
        osm_platform_gpio_set(&_comms_direct_ctx.comms_tx_ports_n_pins, cmd_rx);
        _comms_direct_ctx.prev_cmd_rx = cmd_rx;
        _comms_direct_ctx.last_msg_time = now;
    }
    if (comms_rx != _comms_direct_ctx.prev_comms_rx)
    {
        osm_platform_gpio_set(&_comms_direct_ctx.cmd_tx_ports_n_pins, comms_rx);
        _comms_direct_ctx.prev_comms_rx = comms_rx;
        _comms_direct_ctx.last_msg_time = now;
    }

    return osm_since_boot_delta(osm_get_since_boot_ms(), _comms_direct_ctx.last_msg_time) > COMMS_DIRECT_TIMEOUT_MS;
}


static void _comms_direct_preamble(void)
{
    _comms_direct_ctx.last_msg_time = osm_get_since_boot_ms();

    osm_log_out("Entering COMMS_DIRECT mode");
