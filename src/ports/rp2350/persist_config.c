...
_Static_assert(sizeof(persist_storage_t) <= FLASH_PAGE_SIZE * 8, "Persistent memory too large.");
_Static_assert(sizeof(persist_measurements_storage_t) <= FLASH_PAGE_SIZE * 8, "Persistent measurements too large.");
...
bool persistent_init(void)
{
    ...
    if (wipe)
    {
         ...
         * a packed member
         */
        persist_model_config_t model_config = persist_data.model_config;
        model_persist_config_model_init(&model_config);
...
static bool _persist_data_cmp(void)
{
     ...
     */
    persist_model_config_t model_config = persist_data.model_config;
    persist_model_config_t model_config_raw = persist_data_raw->model_config;
    return !(
        persist_data_raw                                                &&
