...
{
    return _at_poe_ctx.mem && osm_at_mqtt_mem_is_valid();
}
...
void osm_at_poe_init(void)
{
    ...
    osm_at_mqtt_init(&_at_poe_ctx.mqtt_ctx);
    _at_poe_ctx.state = AT_POE_STATE_OFF;
}
...
static void _at_poe_process_state_off(char* msg, unsigned len)
{
    ...
    if (len >= boot_message_len &&
        osm_is_str(boot_message, msg, boot_message_len))
    {
        _at_poe_printf("ATE0");
...
{
    osm_main_loop_iterate_for(10, _at_poe_sleep_loop_iterate, NULL);
}
...
static void _at_poe_process_state_wait_timestamp(char* msg, unsigned len)
{
    ...
    if (osm_is_str(sys_ts_msg, msg, sys_ts_msg_len))
    {
        ...
        _at_poe_ctx.mqtt_ctx.at_base_ctx.time.sys = osm_get_since_boot_ms();
    }
    else if (osm_at_base_is_ok(msg, len))
    {
        _at_poe_ctx.state = AT_POE_STATE_IDLE;
...
void osm_at_poe_process(char* msg)
{
...
{
    if (osm_since_boot_delta(osm_get_since_boot_ms(), _at_poe_ctx.mqtt_ctx.at_base_ctx.last_sent) > AT_POE_TIMEOUT_MS_MQTT)
    {
...
{
    if (osm_since_boot_delta(osm_get_since_boot_ms(), _at_poe_ctx.mqtt_ctx.at_base_ctx.last_sent) > AT_POE_MQTT_FAIL_CONNECT_TIMEOUT_MS)
    {
        /* restart MQTT config */
        _at_poe_do_mqtt_user_conf();
    }
...
void osm_at_poe_loop_iteration(void)
{
    ...
    switch (_at_poe_ctx.state)
    {
        case AT_POE_STATE_OFF:
        {
            uint32_t delta = osm_since_boot_delta(now, _at_poe_ctx.mqtt_ctx.at_base_ctx.off_since);
        ...
        case AT_POE_STATE_DISABLE_ECHO:
            /* fall through */
        case AT_POE_STATE_MQTT_IS_CONNECTED:
            if (osm_since_boot_delta(now, _at_poe_ctx.mqtt_ctx.at_base_ctx.last_sent) > AT_POE_TIMEOUT)
            {
                _at_poe_reset();
            }
            break;
        case AT_POE_STATE_SNTP_WAIT_SET:
            if (osm_since_boot_delta(now, _at_poe_ctx.mqtt_ctx.at_base_ctx.last_sent) > AT_POE_SNTP_TIMEOUT)
            {
                _at_poe_reset();
            }
            break;
...
{
    return osm_at_mqtt_get_id(str, len);
}
...
static command_response_t _at_poe_send_cb(char * args, cmd_ctx_t * ctx)
{
    ...
    return osm_at_base_send_str(pos) ? COMMAND_RESP_OK : COMMAND_RESP_ERR;
}
...
command_response_t osm_at_poe_cmd_config_cb(char * args, cmd_ctx_t * ctx)
{
    ...
    command_response_t ret = osm_at_base_config_setup_str(_at_poe_config_cmds, osm_skip_space(args), ctx);
    if (_at_poe_mem_is_valid())
    {
        ...
        else if (osm_at_poe_persist_config_cmp(&before_config, _at_poe_ctx.mem))
        {
            _at_poe_start();
        }
    }
...
struct cmd_link_t* osm_at_poe_add_commands(struct cmd_link_t* tail)
{
    static struct cmd_link_t cmds[] =
    {
        ...
        { "osm_comms_reset",  "Enable/disable reset line"   , _at_poe_reset_cb         , false , NULL },
