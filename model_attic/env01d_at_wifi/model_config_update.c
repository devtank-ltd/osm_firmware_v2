...
static void _model_config_update_data_v3_to_v4(const osm_persist_model_config_v3_t* v3, osm_persist_model_config_v4_t* v4)
{
    memset(v4, 0, sizeof(osm_persist_model_config_v4_t));
    v4->mins_interval = v3->mins_interval;
    memcpy(&v4->modbus_bus, &v3->modbus_bus, sizeof(osm_modbus_bus_t));
    memcpy(&v4->comms_config, &v3->comms_config, sizeof(osm_comms_config_t));
    memcpy(v4->cc_configs, v3->cc_configs, sizeof(osm_cc_config_t) * ADC_CC_COUNT);
...
bool osm_model_config_update(const void* from_config, osm_persist_model_config_t* to_config, uint16_t from_model_version)
{
    ...
    {
        const osm_persist_model_config_v3_t* v3 = (const osm_persist_model_config_v3_t*)from_config;
