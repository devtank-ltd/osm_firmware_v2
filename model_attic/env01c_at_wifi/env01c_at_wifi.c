...
uint8_t env01c_at_wifi_stm_adcs_get_channel(adcs_type_t adcs_type)
{
    switch(adcs_type)
    {
        case ADCS_TYPE_BAT: return ENV01C_AT_WIFI_ADC1_CHANNEL_BAT_MON;
        case ADCS_TYPE_CC_CLAMP1: return ENV01C_AT_WIFI_ADC1_CHANNEL_CURRENT_CLAMP_1;
        case ADCS_TYPE_CC_CLAMP2: return ENV01C_AT_WIFI_ADC1_CHANNEL_CURRENT_CLAMP_2;
        case ADCS_TYPE_CC_CLAMP3: return ENV01C_AT_WIFI_ADC1_CHANNEL_CURRENT_CLAMP_3;
        default:
            break;
    }
    return 0;
...
void env01c_at_wifi_persist_config_model_init(persist_env01c_at_wifi_config_v1_t* model_config)
{
    model_config->mins_interval = 5 * 1000 / 60;
 ...
 *        false if same      */
bool env01c_at_wifi_persist_config_cmp(persist_env01c_at_wifi_config_v1_t* d0, persist_env01c_at_wifi_config_v1_t* d1)
{
    return !(
        d0 && d1 &&
...
static void _env01c_at_wifi_core_3v3_init(void)
{
...
void env01c_at_wifi_sensors_init(void)
{
    _env01c_at_wifi_core_3v3_init();
    timers_init();
    ios_init();
    sai_init();
    adcs_init();
...
void env01c_at_wifi_post_init(void)
{
    io_watch_init();
}


bool env01c_at_wifi_uart_ring_done_in_process(unsigned uart, ring_buf_t * ring)
{
    if (uart == EXT_UART)
    {
...
bool env01c_at_wifi_uart_ring_do_out_drain(unsigned uart, ring_buf_t * ring)
{
    if (uart == EXT_UART)
...
bool env01c_at_wifi_measurements_get_inf(measurements_def_t * def, measurements_data_t* data, measurements_inf_t* inf)
{
    if (!def || !inf)
    {
...
void env01c_at_wifi_measurements_repopulate(void)
{
    ...
    measurements_repop_indiv(MEASUREMENTS_PM10_NAME,              0,  1, PM10            );
    measurements_repop_indiv(MEASUREMENTS_PM25_NAME,              0,  1, PM25            );
    ...
    measurements_repop_indiv(MEASUREMENTS_W1_PROBE_NAME_2,        0,  1, W1_PROBE        );
    ...
    measurements_repop_indiv(MEASUREMENTS_BATMON_NAME,            5,  1, BAT_MON         );
    ...
    measurements_repop_indiv(MEASUREMENTS_LIGHT_NAME,             4,  5, LIGHT           );
    measurements_repop_indiv(MEASUREMENTS_SOUND_NAME,             4,  5, SOUND           );
}


void env01c_at_wifi_cmds_add_all(struct cmd_link_t* tail)
{
...
void env01c_at_wifi_w1_pulse_enable_pupd(unsigned io, bool enabled)
{
...
bool env01c_at_wifi_can_io_be_special(unsigned io, io_special_t special)
{
...
void env01c_at_wifi_uarts_setup(void)
{
...
void env01c_at_wifi_setup_pulse_pupd(uint8_t* pupd)
{
...
unsigned env01c_at_wifi_measurements_add_defaults(measurements_def_t * measurements_arr)
{
    if (!measurements_arr)
        return 0;
    unsigned pos = 0;
    ...
    measurements_setup_default(&measurements_arr[pos++], MEASUREMENTS_PM10_NAME,            0,  1,  PM10            );
    measurements_setup_default(&measurements_arr[pos++], MEASUREMENTS_PM25_NAME,            0,  1,  PM25            );
    ...
    measurements_setup_default(&measurements_arr[pos++], MEASUREMENTS_W1_PROBE_NAME_2,      0,  1,  W1_PROBE        );
    ...
    measurements_setup_default(&measurements_arr[pos++], MEASUREMENTS_BATMON_NAME,          5,  1,  BAT_MON         );
    ...
    measurements_setup_default(&measurements_arr[pos++], MEASUREMENTS_LIGHT_NAME,           4,  5,  LIGHT           );
    measurements_setup_default(&measurements_arr[pos++], MEASUREMENTS_SOUND_NAME,           4,  5,  SOUND           );
    return pos;
}
