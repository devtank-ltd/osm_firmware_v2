...
at_base_cmd_t* at_mqtt_publish_prep(const char* topic, char* message, unsigned message_len)
{
    ...
    if (_at_mqtt_ctx)
    {
        ...
        ret->len = snprintf(
            ...
            AT_MQTT_LINK_ID,
            AT_BASE_SANIT_STR(full_topic, sizeof(full_topic)),
            message_len,
            AT_MQTT_QOS,
            AT_MQTT_RETAIN
            );
...
at_base_cmd_t* at_mqtt_get_mqtt_user_cfg(void)
{
    ...
    if (_at_mqtt_ctx)
    {
        ...
        ret->len = snprintf(
            ...
            AT_MQTT_CERT_KEY_ID,
            AT_MQTT_CA_ID,
            AT_BASE_SANIT_STR(_at_mqtt_ctx->mem->path, sizeof(_at_mqtt_ctx->mem->path))
            );
        ret->str[ret->len] = 0;
    }
    return ret;
...
at_base_cmd_t* at_mqtt_get_mqtt_sub_cfg(void)
{
    ...
    if (_at_mqtt_ctx)
    {
        ...
        ret->len = snprintf(
            ...
            AT_MQTT_LINK_ID,
            AT_BASE_SANIT_STR(_at_mqtt_ctx->topic_header, sizeof(_at_mqtt_ctx->topic_header)),
            AT_MQTT_QOS
            );
        ret->str[ret->len] = 0;
    }
...
at_base_cmd_t* at_mqtt_get_mqtt_conn(void)
{
    ...
    if (_at_mqtt_ctx)
    {
        ...
        ret->len = snprintf(
            ...
            AT_MQTT_LINK_ID,
            AT_BASE_SANIT_STR(_at_mqtt_ctx->mem->addr, sizeof(_at_mqtt_ctx->mem->addr)),
