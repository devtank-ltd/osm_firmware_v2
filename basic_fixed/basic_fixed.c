...
int basic_fixed_to_str(basic_fixed_t * v, char * buffer, unsigned size)
{
    ...
    return snprintf(buffer, size, "%s%"PRId64".%0"STR(BFP_LOWERLEN)"u", (v->sign)?"-":"", major, minor);
}
...
bool basic_fixed_div(basic_fixed_t * v, basic_fixed_t * a,  basic_fixed_t * b)
{
    ...
    else
    {
        ...
        uint64_t big_reci_frac = big_reciprocal / (BFP_LOWEROVER * 10);

        basic_fixed_t bf_reci = {.sign  = 0,
                                 .upper = big_reci_frac / (BFP_LOWEROVER * 10),
                                 .lower = big_reci_frac % (BFP_LOWEROVER * 10),
                                };
